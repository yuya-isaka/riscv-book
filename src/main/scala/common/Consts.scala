...
object Consts {
  ...
  val BUBBLE        = 0x00000013.U(WORD_LEN.W)  // [ADDI x0,x0,0] = BUBBLE → NOP命令 (何もしない命令ならなんでもいい XOR x0, x0, x0でも，RISC-Vではハードウェア効率の観点で，ADDI命令をNOPとして採用している．)
