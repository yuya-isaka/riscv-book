// BitPatオブジェクトを使って，各命令のbit列を定義
// Instructionsクラスで定義されたメンバーは全て固定値のため，１つのインスタンスを生成するだけで十分．よってシングルトンオブジェクトを用いる．

// ここで定義した命令が，本書での実装対象
// CPUの基礎となる命令，riscv-testと呼ばれるテストコードやCプログラムを実行するために最低限必要な命令
