...
class Core extends Module {
	...
	val stall_flg = Wire(Bool()) // データハザード用
	...
	val pc_next 	= MuxCase(pc_plus4, Seq(
		...
		stall_flg				-> if_reg_pc // ストール時にPCをカウントアップせずに現PCを維持
	))
	if_reg_pc := pc_next
	...
	// データハザード
	id_reg_pc 	:= Mux(stall_flg, id_reg_pc, if_reg_pc) // 次のサイクルIDステージで同じ命令を再処理できるようにする
	// 分岐ハザード
	// id_reg_inst := Mux((exe_br_flg || exe_jmp_flg), BUBBLE, if_inst)   // n
	id_reg_inst := MuxCase(if_inst, Seq(
		// 優先順位重要（ジャンプ処理を優先させて，BUBBLEにする）
	...
	// データハザード（ID/EX間，こっちはめんどくさい）
	val id_rs1_addr_b = id_reg_inst(19, 15) // 一時的にデコード
	val id_rs2_addr_b = id_reg_inst(24, 20) // 一時的にデコード
	...
	// io.dmem.rdataを接続する関係で移行（データハザードのフォワーディングで使う）
	...
	io.gp := regfile(3)
	// io.exit := (id_reg_inst === UNIMP) // ChiselTest
	io.exit := (mem_reg_pc === 0x44.U(WORD_LEN.W)) // riscv-tests
